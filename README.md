...
It is highly recommended that the user takes advantadge of this feature.  The other option is for the user to generate a new ethereum address on their own (e.g. mycrypto, mew, metamask, rainbow, trust wallet, etc.).  If the user does not generate a new, unused address and instead withdraws the ETH to a previously used ethereum address, privacy guarantees are compromised.
